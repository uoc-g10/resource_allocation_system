...
$sked3.on('event:dragEnded.skedtape', function(e) {
console.log(e.detail.event);
});

$sked3.on('event:click.skedtape', function(e) {
if (e.detail.event.locationId == '1') {
    alert(e.detail.event.id);
}

//$sked3.skedTape('removeEvent', e.detail.event.id);
});

console.log(today(6, 0));

function newReservation() {
$sked3.skedTape('startAdding', {
    name: 'New meeting',
    location: '1',
    start: today(6, 0),
    end: today(7, 0),
...
$sked3.on('timeline:click.skedtape', function(e, api) {
try {
    newReservation();
} catch (e) {
