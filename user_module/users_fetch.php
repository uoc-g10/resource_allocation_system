...
$query = "SELECT id,title,firstname,secondname,email,password,mobile,role,department,last_login,status FROM users ";

if (isset($_POST["search"]["value"])) {
    $query .= ' WHERE id LIKE "%' . $_POST["search"]["value"] . '%" OR firstname LIKE "%' . $_POST["search"]["value"] . '%" OR secondname LIKE "%' . $_POST["search"]["value"] . '%" ';
}
...
while ($row = mysqli_fetch_array($result)) {
    ...
    $sub_array[] = "<div class='text-right'><button class='btn btn-default btn-sm btn-flat'><i class='fa fa-ban'></i> Block</button> 
